...
const IDBotDao_1 = require("./IDBotDao");
...
class SqliteDBotDao {
    addMessageActivity(message, callback) {
        Database_1.DB.sqlite.serialize(() => {
            Database_1.DB.sqlite.run(addMessageSql, [
                ...
                message.channel,
                message.username,
            ], IDBotDao_1.AdaptAddRecordCallback(callback));
        });
    }
    ...
    getAllMessages(callback) {
        Database_1.DB.sqlite.serialize(() => {
            Database_1.DB.sqlite.all(getAllMessagesSql, [], IDBotDao_1.AdaptRowMapping((rows) => {
                return rows.map((r) => {
                    return {
                        id: r.id,
                        content: r.content,
    ...
    getAllVoiceActivities(callback) {
        Database_1.DB.sqlite.serialize(() => {
            Database_1.DB.sqlite.all(getAllVoiceActivitiesSql, [], IDBotDao_1.AdaptRowMapping((rows) => {
                return rows.map((r) => {
                    return {
                        id: r.id,
                        username: r.username,
